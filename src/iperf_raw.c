...
/* iperf_raw_recv
 *
 * receives the data for RAW
 */
int
iperf_raw_recv(struct iperf_stream *sp)
{
    uint32_t  sec, usec;
    uint64_t  pcount;
    int       r;
...
/*
 * The following functions all have to do with managing RAW data sockets.
 * RAW of course is connectionless, so there isn't really a concept of
 ...
 */
int
iperf_raw_buffercheck(struct iperf_test *test, int s)
{
    int rc = 0;
 ...
 * iperf_raw_accept
 *
 * Accepts a new RAW "connection"
 */
int
iperf_raw_accept(struct iperf_test *test)
{
    ...
    rc = iperf_raw_buffercheck(test, s);
    if (rc < 0)
	/* error */
	return rc;
    /*
    ...
    /* Let the client know we're ready "accept" another RAW "stream" */
 ...
 * iperf_raw_listen
 *
 * Start up a listener for RAW stream connections.  Unlike for TCP,
 * there is no listen(2) for RAW.  This socket will however accept
 * a RAW datagram from a client (indicating the client's presence).
 */
int
iperf_raw_listen(struct iperf_test *test)
{
    int s;
 ...
 * iperf_raw_connect
 *
 * "Connect" to a RAW stream listener.
 */
int
iperf_raw_connect(struct iperf_test *test)
{
    int s, sz;
    unsigned int buf;
#ifdef SO_RCVTIMEO
    ...
    /*
     * Write a datagram to the RAW stream to let the server know we're here.
    ...
    do {
        ...
        i += sz;
    } while (buf != RAW_CONNECT_REPLY && buf != LEGACY_RAW_CONNECT_REPLY && i < max_len_wait_for_reply);

    if (buf != RAW_CONNECT_REPLY  && buf != LEGACY_RAW_CONNECT_REPLY) {
        i_errno = IESTREAMREAD;
        return -1;
    }
...
/* iperf_raw_init
 *
 * initializer for RAW streams in TEST_START
 */
int
